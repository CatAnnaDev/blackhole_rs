...
fn sigma(r: f32, ct: f32, a: f32) -> f32 {
...
fn deriv(state: vec4<f32>, phi: f32, e: f32, l: f32, a: f32) -> vec4<f32> {
    let r = state.x;
    let th = state.y;
    let pr = state.z;
    let pth = state.w;
    ...
    let dpr = -(hamiltonian(r + hr, th, pr, pth, e, l, a)
                - hamiltonian(r - hr, th, pr, pth, e, l, a)) / (2.0 * hr);
    let dpth = -(hamiltonian(r, th + hth, pr, pth, e, l, a)
                 - hamiltonian(r, th - hth, pr, pth, e, l, a)) / (2.0 * hth);
...
fn init_geodesic(ro: vec3<f32>, rd: vec3<f32>, a: f32) -> Geo {
    ...
    let a2 = a * a;
    let r2 = 0.5 * ((big_r2 - a2) + sqrt(max((big_r2 - a2) * (big_r2 - a2) + 4.0 * a2 * ro.y * ro.y, 0.0)));
    ...
    var n = vec3<f32>(dot(rd, er), dot(rd, eth), dot(rd, eph));
    n = normalize(n);
...
fn rk4(state: vec4<f32>, phi: f32, h: f32, e: f32, l: f32, a: f32) -> vec4<f32> {
    let k1 = deriv(state, phi, e, l, a);
    let k2 = deriv(state + 0.5 * h * k1, phi, e, l, a);
    let k3 = deriv(state + 0.5 * h * k2, phi, e, l, a);
    let k4 = deriv(state + h * k3, phi, e, l, a);
...
fn disk_color(r: f32, e: f32, l: f32, a: f32, r_in: f32, r_out: f32) -> vec3<f32> {
    let flux = page_thorne(r, a, r_in);
    let fref = max(page_thorne(r_in * 2.2, a, r_in), 1.0e-3);
    let fnorm = clamp(flux / fref, 0.0, 24.0);

    let t_emit = cam.disk.y * pow(fnorm, 0.25);
    let g = clamp(disk_redshift(r, e, l, a), 0.05, 4.0);
    let t_obs = clamp(t_emit * g, 900.0, 42000.0);

    let bb = blackbody(t_obs);
    let beaming = pow(g, 3.0);
    let radiance = pow(fnorm, 0.85) * beaming * cam.disk.x;
    let edge = smoothstep(r_in, r_in + 0.4, r)
        * smoothstep(r_out, r_out - 6.0, r);
...
fn trace(ro: vec3<f32>, rd: vec3<f32>, a: f32) -> vec3<f32> {
    ...
    let r_in = isco(a);
    let r_out = cam.disk.z;

    var prev = ro;
    var captured = false;

    let max_steps = i32(cam.frame.w);
    for (var i = 0; i < max_steps; i = i + 1) {
        ...
        let dist = r - rh;
        let h = clamp(0.06 * min(dist, r), 0.004, 1.5);

        let new_state = rk4(geo.state, geo.phi, h, geo.e, geo.l, a);
        ...
        if (s_old * s_new < 0.0) {
            ...
            if (r_cross >= r_in && r_cross <= r_out) {
                return disk_color(r_cross, geo.e, geo.l, a, r_in, r_out);
            }
        }
    ...
    let out_dir = normalize(now - prev);
    return starfield(out_dir);
}

@compute @workgroup_size(8, 8, 1)
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    let w = u32(cam.params.z);
    let h = u32(cam.params.w);
    if (gid.x >= w || gid.y >= h) {
        return;
    }

    let jitter = vec2<f32>(cam.frame.y, cam.frame.z);
    let px = (vec2<f32>(gid.xy) + vec2<f32>(0.5) + jitter) / vec2<f32>(f32(w), f32(h));
    let ndc = vec2<f32>(px.x * 2.0 - 1.0, 1.0 - px.y * 2.0);

    let tan_half = cam.params.x;
    let aspect = cam.params.y;
    let dir = normalize(
        cam.fwd.xyz
        + cam.right.xyz * (ndc.x * aspect * tan_half)
        + cam.up.xyz * (ndc.y * tan_half)
    );

    let a = clamp(cam.misc.y, 0.0, 0.999);
    let color = trace(cam.pos.xyz, dir, a) * cam.disk.w;

    let coord = vec2<i32>(gid.xy);
    let n = cam.frame.x;
    var accum = color;
    if (n > 0.5) {
        let prev = textureLoad(prev_tex, coord, 0).rgb;
        accum = mix(prev, color, 1.0 / (n + 1.0));
    }
    textureStore(out_tex, coord, vec4<f32>(accum, 1.0));
}
